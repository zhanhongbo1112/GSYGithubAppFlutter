...
import 'package:yqboots/src/widgets/menu/src/arc_progress_indicator.dart';
import 'package:yqboots/src/widgets/menu/src/radial_menu_button.dart';
import 'package:yqboots/src/widgets/menu/src/radial_menu_center_button.dart';
import 'package:yqboots/src/widgets/menu/src/radial_menu_item.dart';
...
class _RadialMenuLayout extends MultiChildLayoutDelegate {
  ...
  _RadialMenuLayout({
    ...
    this.controller,
  }) : _progress = Tween<double>(begin: 0.0, end: radius)
            .animate(CurvedAnimation(curve: Curves.elasticInOut, parent: controller));
    ...
    for (int i = 0; i < itemCount; i++) {
      ...
      if (hasChild(actionButtonId)) {
        ...
        positionChild(
          actionButtonId,
          Offset(
            (center.dx - buttonSize.width) + (_progress.value) * Math.cos(itemAngle),
            (center.dy - buttonSize.height) + (_progress.value * (i + 1)) * Math.sin(itemAngle),
          ),
        );
      }
    }
