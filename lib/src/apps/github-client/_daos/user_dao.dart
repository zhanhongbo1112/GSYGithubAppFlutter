...
class UserDao {
  static login(userName, password, store) async {
    ...
    var res = await httpManager.netFetch(
        GitHubClientApis.getAuthorization(), json.encode(requestParams), null, Options(method: "post"));
    var resultData = null;
  ...
  static getNotifyDao(bool all, bool participating, page) async {
    ...
    var res = await httpManager.netFetch(url, null, null, null);
    if (res != null && res.result) {
      List<GitHubNotification> list = List();
      var data = res.data;
      if (data == null || data.length == 0) {
        return DataResult([], true);
      }
      for (int i = 0; i < data.length; i++) {
        list.add(GitHubNotification.fromJson(data[i]));
      }
      return DataResult(list, true);
    } else {
      return DataResult(null, false);
